...
public class MachineRepairedEventHandler(IHubContext<WarehouseHub> hub) : IEventHandler<MachineRepairedEvent>
{
    public async Task Handle(MachineRepairedEvent @event)
    {
        await hub.Clients.All.SendAsync("MachineStatusUpdated", new MachineStatusUpdate(
            @event.MachineId,
            @event.MachineType.ToString(),
            MachineStatus.Running.ToString(),
            null
        ));
    }
}
